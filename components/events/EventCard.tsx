...
import { Calendar, Clock, MapPin, Users, ArrowUpRight, Sparkles } from "lucide-react";
            ...
            <div className="mt-4 pt-3 border-t border-white/5 space-y-1.5 text-xs text-slate-400 font-mono">
              ...
              <div className="flex items-center justify-between text-[11px]">
                <div className="flex items-center gap-1.5 text-slate-400">
                  ...
                  <span>{event.currentRegistrations} / {event.maxSeats} registered</span>
                </div>
                <div className="w-20 bg-navy-950 rounded-full h-1.5 overflow-hidden border border-white/5">
                  <div
                    className="bg-aws-orange h-full rounded-full"
                    style={{ width: `${Math.min(100, (event.currentRegistrations / event.maxSeats) * 100)}%` }}
                  />
                </div>
              </div>
            </div>
