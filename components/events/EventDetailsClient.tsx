...
import { Calendar, Clock, MapPin, Users, Sparkles, Share2, Check } from "lucide-react";
...
export function EventDetailsClient({ event, formattedDate }: Props) {
  ...
  const percentFull = Math.min(100, Math.round((event.currentRegistrations / event.maxSeats) * 100));
          ...
          <div className="pt-2 border-t border-white/10 space-y-1.5">
            <div className="flex items-center justify-between text-[11px]">
              ...
              <span className="font-bold text-white">
                {event.currentRegistrations} / {event.maxSeats} ({percentFull}%)
              </span>
            </div>
            <div className="w-full bg-navy-950 rounded-full h-2 overflow-hidden border border-white/10">
              <div
                className="bg-gradient-to-r from-amber-500 to-aws-orange h-full rounded-full transition-all duration-500"
